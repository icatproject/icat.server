...
, CONSTRAINT KEYWORD_PK PRIMARY KEY 
  (
    INVESTIGATION_ID 
  , NAME 
  )
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT INSTRUMENT_TYPE_PK PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009839 PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009833 PRIMARY KEY 
  (
    NAME 
  , VERSION 
  )
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009843 PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT PK_FGDF PRIMARY KEY 
  (
    SOURCE_DATAFILE_ID 
  , DEST_DATAFILE_ID 
  )
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT STUD_INVESTIGATION_PK PRIMARY KEY 
  (
    STUDY_ID 
  , INVESTIGATION_ID 
  )
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009845 PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009868 PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) DEFAULT 'Y' NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT ICAT_ROLE_PK PRIMARY KEY 
  (
    ROLE 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT PK_INVR PRIMARY KEY 
  (
    INVESTIGATION_ID 
  , FACILITY_USER_ID 
  )
...
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009838 PRIMARY KEY 
  (
    NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT FACILITY_USER_PK PRIMARY KEY 
  (
    FACILITY_USER_ID 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) DEFAULT 'Y' NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT ICAT_AUTHORISATION_PK PRIMARY KEY 
  (
    ID 
  )
  ENABLE 
...
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, FACILITY_ACQUIRED VARCHAR2(1) DEFAULT 'Y' NOT NULL 
, CONSTRAINT THIS_ICAT_PK PRIMARY KEY 
  (
    FACILITY_SHORT_NAME 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT SYS_C009866 PRIMARY KEY 
  (
    ID 
  )
  ENABLE 
...
, FACILITY_ACQUIRED VARCHAR2(1) NOT NULL 
, DELETED VARCHAR2(1) DEFAULT 'N' NOT NULL 
, CONSTRAINT FACILITY_INSTRUMENT_SCIEN_PK PRIMARY KEY 
  (
    INSTRUMENT_NAME 
  , FEDERAL_ID 
  )
...
COMMENT ON TABLE SAMPLE IS 'This table stores the sample information for both the experiment (abstract) and the datasets (instances) e.g. in the case of the information from the proposal system the abstract sample information should have the instance column set to NULL.

It should be noted that Samples are often substituted if the stated samples instances do not produce good results. Scientists usually have reserve samples or other samples which they want to try out (this usually happens half of the time as an estimate on ISIS) this can happen due to a variety of reasons:

1) The sample instances is not producing good results.
2) The experiment is progressing better than expected and the experimental team have time left to look at a reserve sample.

This is not usually a problem if the sample safety characteristics are the same; usually the will just substitute the sample. 

However if the substituted samples:
1) Have very different sample safety characteristics 
2) Are totally different samples from the original and they person is attempting to queue jump by analysing a particularly "hot" (from a research perspective) compound.

Then a station scientist must be asked/involved as

1) Sample can can explode and the people going in to clear up need to know if these are for example radioactive
2) This is queue jumping and is attempted often but is considered to be unfair play and is actively stopped';
...
COMMENT ON COLUMN DATAFILE_PARAMETER.DATETIME_VALUE IS 'Store date and time values of parameter';
...
COMMENT ON COLUMN PARAMETER.NUMERIC_VALUE IS 'Y or y denote that the value of the parameter is a number - N or n denotes that it is a string and D or d denote that the value of the parameter is Date and Time.

Future version of this will be changed to include a more accurate representation instead of Y, N or D';
...
COMMENT ON COLUMN DATASET.DATASET_TYPE IS 'The rules governing what dataset type is given for a particular investigation type are facility specific. This may be an example of a business rule which would be of use in a rule engine. An Example of the rule is the context of the facilities at STFC is: 
If INVESTIGATION.INV_TYPE == EXPERIMENT or
INVESTIGATION.INV_TYPE == CALIBRATION 
then dataset type could be any of the following:
1) PRE_EXPERIMENT_DATA
2) DETECTOR_CALIBRATION (one detector)
3) EXPERIMENT_CALIBRATION (with a calibration sample)
4) EXPERIMENT_RAW
5) LASER_SHOT (CLF specific)
6) LASER_DIAGNOSTICS (CLF specific)
7) TARGET_DATA
8) SIMULATION
9) ANALYSIS 
...
COMMENT ON COLUMN DATASET_PARAMETER.DATETIME_VALUE IS 'Stores Date and Time value of parameter';
