...
--------------------------------------------------------------------------------
        PROCEDURE propagate_data (beamline varchar2);
...
PROCEDURE propagate_data(beamline varchar2) IS

  x INTEGER;
  LV_LOCKNAME CONSTANT VARCHAR2(50) := 'POPULATE_BEAMLINES_PKG';--.propagate_data';
  lv_lockhandle VARCHAR2(100);
  dblink BEAMLINE_INSTRUMENT.DBLINK%TYPE;

BEGIN
  log_pkg.init;
  log_pkg.write_log('Propagating data to beamline databases ON DEMAND');
  ...
  ELSE
      BEGIN
        log_pkg.write_log('');
        log_pkg.write_log('***');
        log_pkg.write_log('Instrument: '||beamline);
        
        select dblink into dblink from beamline_instrument where instrument=lower(beamline);
        
        -- synonyms
        create_synonyms(dblink);
        ...
        -- application data
        push_investigation(beamline);
        push_shift(beamline);
        push_facility_user(beamline);
        push_investigator(beamline);
        push_keyword(beamline);
        push_publication(beamline);
        push_sample(beamline);
        push_dataset(beamline);
        push_sample_parameter(beamline);


        log_pkg.write_log('Propagation successful for instrument '||beamline);
      EXCEPTION
        WHEN OTHERS THEN
          log_pkg.write_exception(
            'Propagation failed for instrument '||beamline||Chr(10)||SQLERRM);
