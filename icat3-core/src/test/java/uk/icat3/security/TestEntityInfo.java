...
public class TestEntityInfo {
	...
	private static EntityInfoHandler pkHandler = EntityInfoHandler
			.getInstance();
	...
	public void testConstraints() throws Exception {
		testConstraint(Investigation.class, "name", "visitId", "facilityCycle",
				"instrument");
	...
	private void testConstraint(Class<? extends EntityBaseBean> klass,
			String... name) throws IcatInternalException {
		...
		if (name.length == 0) {
			...
			assertEquals(klass.getSimpleName() + " count", name.length,
					result.size());

			int i = 0;
			for (Field re : result) {
				assertEquals(klass.getSimpleName() + " value " + i, name[i++],
						re.getName());
			}
		}
	}

	private void testPKS(Class<? extends EntityBaseBean> klass, String pkname)
			throws Exception {
	...
	public void testRels() throws Exception {
		...
		testRel(Investigation.class, "Instrument by instrument one",
				"Keyword by keywords many cascaded",
				"Sample by samples many cascaded",
				"TopicInvestigation by topicInvestigations many cascaded",
				"StudyInvestigation by studyInvestigations many cascaded",
				"Shift by shifts many cascaded",
				"Dataset by datasets many cascaded",
		...
		testRel(Dataset.class, "InputDataset by inputDatasets many cascaded",
				...
				"Datafile by datafiles many cascaded",
				"OutputDataset by outputDatasets many cascaded",
				"DatasetStatus by status one", "DatasetType by type one",
				"Sample by sample one");

		testRel(Keyword.class, "Investigation by investigation one");

		testRel(InvestigationUser.class, "Investigation by investigation one",
				"User by user one");
	...
	private void testRel(Class<? extends EntityBaseBean> klass, String... rels)
			throws Exception {
		...
		assertEquals(klass.getSimpleName() + " count", rels.length,
				results.size());
		for (String rel : rels) {
			assertTrue(klass.getSimpleName() + " value " + rel,
					rStrings.contains(rel));
		}
	}
	...
	private void testNNF(Class<? extends EntityBaseBean> klass, String... nnfs)
			throws Exception {
		...
		assertEquals(klass.getSimpleName() + " count", nnfs.length,
				results.size());

		for (String nnf : nnfs) {
			assertTrue(klass.getSimpleName() + " value " + nnf,
					rStrings.contains(nnf));
		}
	}

	@Test
	public void stringFields() throws Exception {
		testSF(Investigation.class, "visitId 255", "summary 4000", "name 255",
				"title 255");
		...
		testSF(ParameterType.class, "description 255", "unitsFullName 255",
				"units 255", "name 255");
		testSF(Job.class);

	}

	private void testSF(Class<? extends EntityBaseBean> klass, String... sfs)
			throws Exception {
		...
		assertEquals(klass.getSimpleName() + " count", sfs.length,
				results.size());

		for (String sf : sfs) {
			assertTrue(klass.getSimpleName() + " value " + sf,
					rStrings.contains(sf));
		}
	}
	...
	private void testGetters(Class<? extends EntityBaseBean> klass, int count)
			throws Exception {
		...
		for (Entry<Field, Method> entry : results.entrySet()) {
			...
			assertTrue(klass.getSimpleName() + " value ", m.equals("get" + cap)
					|| m.equals("is" + cap));
		}
	...
	private void testSetters(Class<? extends EntityBaseBean> klass, int count)
			throws Exception {
	...
	public void keytype() throws Exception {
		...
		testKeytype(InvestigationUser.class,
				EntityInfoHandler.KeyType.GENERATED);
	...
	private void testKeytype(Class<? extends EntityBaseBean> klass,
			KeyType keyType) throws Exception {
		assertEquals(klass.getSimpleName() + " keyType", keyType,
				pkHandler.getKeytype(klass));
	}
