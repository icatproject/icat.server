...
import static uk.icat3.util.Queries.ALL_FACILITYCYCLES;
...
/**
 * This is the manager class for all operations for facility related objects - facility_users,
 * facility_instrument_scientists and facility_cycles
 * 
 * These are update, remove, delete, create on these objects. <br />
 * <br />
 * 
 * @author tang76
 */
public class FacilityManager  {
	 ...
	 */
	public static FacilityUser getFacilityUserByFederalId(String federalId, EntityManager manager)
			throws NoSuchObjectFoundException {
		log.trace("getFacilityUserByFederalId(String, EntityManager)");

		try {
			FacilityUser user = (FacilityUser) manager.createNamedQuery("FacilityUser.findByFederalId")
					.setParameter("fedId", federalId).getSingleResult();
			return user;
		} catch (Exception e) {
			throw new NoSuchObjectFoundException("" + e);
		}
	...
	public static FacilityUser getFacilityUserByFacilityUserId(String facilityUserId, EntityManager manager) {
		...
		// return
		// manager.createNamedQuery(ALL_FACILITYCYCLES)/*.setMaxResults(MAX_QUERY_RESULTSET)*/.getResultList();
