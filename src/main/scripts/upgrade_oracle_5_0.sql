...
BEGIN
    ...
    ELSIF :NEW.FILESIZE != :OLD.FILESIZE THEN
...
BEGIN
    IF :NEW.INVESTIGATION_ID != :OLD.INVESTIGATION_ID THEN
        DELTA := - NVL(:OLD.DATASETSIZE, 0);
        UPDATE_INVESTIGATION_SIZE(:OLD.INVESTIGATION_ID, DELTA);

        DELTA := NVL(:NEW.DATASETSIZE, 0);
        UPDATE_INVESTIGATION_SIZE(:NEW.INVESTIGATION_ID, DELTA);
    END IF;
END;
/
