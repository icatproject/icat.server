...
public class DownloadServlet extends HttpServlet {
	...
	public void init() {
		...
		if (new File(System.getProperty("user.home") + File.separator + "." + facilityLogFile
				+ "-icatapi.xml").exists()) {
			PropertyConfigurator.configure(System.getProperty("user.home") + File.separator + "."
					+ facilityLogFile + "-icatapi.xml");
			// log.info("Loading "+System.getProperty("user.home")+File.separator+"log4j.xml");
		} else {
			PropertyConfigurator.configure(System.getProperty("user.home") + File.separator + "."
					+ facilityLogFile + "-icatapi.properties");
			// log.info("Loading "+System.getProperty("user.home")+File.separator+"log4j.properties");
		}

		PATH = System.getProperty("java.io.tmpdir") + File.separator + "srbapi_downloads"
				+ File.separator + FACILITY;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (sid == null) {
			...
			try {
				...
				if (e.getType() == IcatExceptionType.SESSION) {
					log.error("Someone trying to download a file with invalid sessionId " + sid
							+ ", IP Addess: " + request.getRemoteAddr());
					// session invalid
					response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
				} else {
					response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e.getType()
							+ " " + e.getMessage());
				}
				return;
			}
		...
		if (file == null) {
			...
			Date today = new Date();
			downloadFile = new File(PATH + File.separator + today.getDate() + "-"
					+ (today.getMonth() + 1) + File.separator + file);
			if (!downloadFile.exists()) {
				log.error("FileNotFound from " + request.getRemoteAddr() + ": File not found "
		...
		try {
			...
			response.setHeader("Content-disposition",
					"attachment; filename=" + name.replaceAll(" ", "_") + ""); // replace all spaces
			...
			if (!downloadFile.getAbsolutePath().endsWith(".zip")) {
				// remove parent
				log.debug("deleting "
						+ downloadFile.getAbsoluteFile().getParentFile().getAbsolutePath() + "?:"
		 ...
		 * concurrent.add();
		 * 
		 * File f = new File(System.getProperty("java.io.tmpdir")+File.separator+"large"); int
		 * length = 0; ServletOutputStream op = response.getOutputStream(); ServletContext context =
		 * getServletConfig().getServletContext(); String mimetype = context.getMimeType(
		 * f.getName() );
		 * 
		 * response.setContentType( (mimetype != null) ? mimetype : "application/octet-stream" );
		 * response.setContentLength( (int)f.length() ); response.setHeader( "Content-Disposition",
		 * "attachment; filename=\"" + f.getName() + "\"" );
		 * 
		 * byte[] bbuf = new byte[1024]; FileInputStream fil =new FileInputStream(f);
		 * DataInputStream in = new DataInputStream(fil); System.out.println("opened buffer"); while
		 * ((in != null) && ((length = in.read(bbuf)) != -1)) { op.write(bbuf,0,length); }
		 * 
		 * log.trace("Concurrent "+concurrent.get()+" : MAX: "+concurrent.getMax());
		 * 
		 * fil.close(); in.close(); op.flush(); op.close();
		 * 
		 * concurrent.minus();
		 */
	}
	 ...
	 * 
	 * @param filename
	 *            name of the file whose type to determine
	...
	public String getContentType(String filename) {
		...
		} else if (ext.equalsIgnoreCase("txt") || ext.equalsIgnoreCase("out")
				|| ext.equalsIgnoreCase("err")) {
